...
if [[ "${dependency_name}" =~ "agent" ]]; then
  ...
  agent_version=$(grep "tag:" "charts/${chart}/values.yaml" | grep "agent" | awk '{print $2}' | tr -d '"')

  # Extract major.minor.patch from current version
  current_major=$(echo "${agent_version}" | cut -d. -f1)
  current_minor=$(echo "${agent_version}" | cut -d. -f2)
  current_patch=$(echo "${agent_version}" | cut -d. -f3)
